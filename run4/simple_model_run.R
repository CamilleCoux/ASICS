...
models_list <- models
models <- list(simple_model = models[[1]])
...
while(Lst <= length(samples_list)){
  ...
  filename.in = file.path(modelDir,paste("simple_model_thin_", as.character(thin),
                                         ...
                                         "_chains_",as.character(nChains),
                                         ".Rdata",sep = ""))
  filename.out = file.path(modelDir,paste("MF_simple_models_thin_", as.character(thin),
                                          "_samples_", as.character(samples),
                                          "_chains_",as.character(nChains),
...
if(file.exists(filename)){
  ...
  for(j in 1:nm){
    ...
    if(m$nr+length(covariates)>1 & m$ns>1){
      ...
      filename = file.path(resultDir, paste("parameter_estimates_simple_model_VP_",modelnames[j],".csv"))
      write.csv(vals,file=filename)
      if(!is.null(VP$R2T$Beta)){
        filename = file.path(resultDir,paste("parameter_estimates_simple_model_VP_R2T_Beta",modelnames[j],".csv"))
        write.csv(VP$R2T$Beta,file=filename)
      }
      if(!is.null(VP$R2T$Y)){
        filename = file.path(resultDir, paste("parameter_estimates_simple_model_VP_R2T_Y",modelnames[j],".csv"))
        write.csv(VP$R2T$Y,file=filename)
      }
      if(all(var.part.order.explained[[j]]==0)){
        c.var.part.order.explained = 1:m$ns
  ...
  for(j in 1:nm){
    ...
    if(m$nc>1){
      postBeta = getPostEstimate(m, parName="Beta")
      filename = file.path(resultDir, paste("parameter_estimates_simple_model_Beta_",modelnames[j],".xlsx"))
      me = as.data.frame(t(postBeta$mean))
      me = cbind(m$spNames,me)
      colnames(me) = c("Species",m$covNames)
      po = as.data.frame(t(postBeta$support))
  ...
  for(j in 1:nm){
    ...
    if(m$nr>0 & m$ns>1){
      ...
      for (r in 1:m$nr){
        ...
        colnames(ne)[1] = ""
        vals = list("Posterior mean"=me,"Pr(x>0)"=po,"Pr(x<0)"=ne)
        filename = file.path(resultDir, paste("parameter_estimates_simple_model_Omega_",modelnames[j],"_",names(m$ranLevels)[[r]],".xlsx"))
        writexl::write_xlsx(vals,path = filename)
      }
    }
  }
